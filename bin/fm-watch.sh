...
# knows WHY this pane is being surfaced - the auto-resume ladder is the only one
# today - passes a fuller reason so the wake itself carries that history instead
# of the supervisor having to rediscover it.
surface_nonterminal_stale() {  # <window> <hash> [reason]
  local win=$1 h=$2 reason=${3:-} key task last declared=1 bounded=1 throttled=1 until now
...
# An mtime in the future also reads as DUE: a clock that stepped backwards must
# not freeze a cadence gate.
age_of() {
  local f=$1 m now

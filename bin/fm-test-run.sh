...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-agent-process-lib.sh)
      ...
      printf '%s\n' pure-contract-unit
      ;;
    bin/fm-watch*|bin/fm-wake*|bin/fm-stall-lib.sh|bin/fm-inactive-reconcile.sh|\
...
run_one_serial() {
  ...
  : "${rc:=1}"
  if [ "$rc" -eq 124 ]; then
    log "script exceeded its ${PER_SCRIPT_TIMEOUT_SECS}s bound and was killed: $script"
  fi

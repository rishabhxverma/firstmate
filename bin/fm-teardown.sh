...
remote_secondmate_teardown() {
  ...
  rm -f -- "$STATE/$ID.turn-ended" "$STATE/$ID.progress" "$STATE/$ID.stall"
...
# own live work in this exact worktree. Not for kind=secondmate: a secondmate
...
[ "$KIND" = secondmate ] || capture_task_reflection

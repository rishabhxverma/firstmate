...
#
# Away mode (state/.afk): see the "Away mode" paragraph below; the beat window
# it applies is AFK_BEAT_GRACE, the poll-cadence-derived grace
...
# Single-sourced from FM_GUARD_GRACE (or the poll-derived grace, resolved after
# bin/fm-wake-lib.sh is sourced) rather than a second knob, for two reasons.
# It has to clear away mode's longest LEGITIMATE beat gap: the daemon runs its
# watcher child on the watcher's own poll cadence (FM_POLL, 15s) but pauses it
# for up to CRASH_BACKOFF (60s) after a crash loop and up to INJECT_FAIL_SLEEP
# (30s) while the escalation target is gone (bin/fm-supervise-daemon.sh owns
# those), so anything near one poll cycle would reintroduce the false alarm this
# check exists to remove; 300s is five of those worst-case pauses. And a second
# knob here could drift from the staleness standard every other supervision
# model in this guard is already judged by, which would make "supervision is
# stale" mean two different things in one banner.
...
# Away-mode beat window: FM_GUARD_GRACE when set, else the poll-cadence-derived
# grace (see the header), resolved here because it needs bin/fm-wake-lib.sh.
AFK_BEAT_GRACE=${FM_GUARD_GRACE:-$(fm_poll_derived_grace)}

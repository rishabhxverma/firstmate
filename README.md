...
```sh
gh auth login
git clone https://github.com/rishabhxverma/firstmate
cd firstmate
```
...
| `/updatefirstmate` | Guardedly update the running firstmate and its secondmates - fast-forward, or reconcile a redundant post-squash-merge divergence - then persist and restart every live mate successfully left on the target commit - including already-current homes - with an honest re-read nudge only when restart cannot be proven; on a fork it also reports, read-only, when the community upstream needs a merge |
...
- `.agents/skills/` - agent-loaded skills (this section's table, plus firstmate's agent-only reference skills). Every firstmate-authored one assumes a live firstmate home and is meaningless, or actively misleading, installed anywhere else, so each carries `metadata.internal: true` in its frontmatter. That flag hides them from installer discovery (tools like the [skills.sh](https://skills.sh) `npx skills add` installer) without affecting how firstmate itself loads them - frontmatter metadata is inert to the agent's own skill loader.
  This directory also holds vendored third-party skills installed from upstream sources and recorded in the root `skills-lock.json`. They are committed byte-identical to what the installer wrote, so this repo's writing conventions and the `metadata.internal` marker do not apply to them and their files are never hand-edited here; fixes belong upstream. Their prose is likewise outside the audience inventory (see [docs/documentation-audiences.md](docs/documentation-audiences.md)).

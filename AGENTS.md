...
1. **Never write to a project.**
   ...
   The only exceptions are the guarded project initialization, fleet sync, secondmate sync and inherited local-material propagation, declared per-project local-environment propagation into a crewmate worktree whose `.env` that project actually gitignores (`bin/fm-project-env-lib.sh`, `bin/fm-spawn.sh`, `bin/fm-project-env-sync.sh`), self-update, and approved `local-only` merge paths, each owned by its referenced skill or script, plus a concrete captain-approved project operation governed directly by this rule.
...
config/auto-resume  Claude stall auto-resume switch; LOCAL, gitignored; absent or "on" = default-on unattended recovery for claude-harness crewmates and scouts, "off" disables it; inherited by secondmate homes; see docs/configuration.md "Claude stall auto-resume"
...
config/project-env/<project>.env  declared per-project local-environment source; LOCAL, gitignored; absent means that project has no shared local environment to propagate. bin/fm-spawn.sh merges it into a ship/scout worktree's .env at spawn time (never clobbering a key the worktree already has), and bin/fm-project-env-sync.sh converges every existing treehouse pool worktree for the project on demand; see bin/fm-project-env-lib.sh
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/reflection.md  finished task's reflection material, harvested by bin/fm-reflect.sh before teardown destroys it; survives teardown, and the task-reflection skill folds it into learnings and deletes it
projects/            cloned repos; gitignored; read-only except under hard rule 1's concrete captain-approved project operation exception
state/               runtime records and signals; gitignored
  <id>.status        appended by crewmates: "<state>: <note>" wake-event lines, not current-state truth
  <id>.turn-ended    touched by turn-end hooks
  <id>.stall         open auto-resume episode for a stalled claude worker: class, attempts, and the next scheduled resume; written only by bin/fm-stall-lib.sh, removed by teardown, safe to delete (forces a fresh episode)
  ...
  .hash-* .count-* .stale-* .stale-since-* .stall-declined-* .churn-since-* .paused-* .wedge-escalations-* .dead-reported-* .writing-* .waiting-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `PROJECT_ENV:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
...
- `task-reflection` - load on a `check: reflection` wake.

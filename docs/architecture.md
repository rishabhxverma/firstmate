...
A Claude worker whose pane shows a usage-limit or upstream-5xx banner is a further exemption, and the only one that acts rather than waits: [`bin/fm-stall-lib.sh`](../bin/fm-stall-lib.sh) schedules a bounded resume and the pane is absorbed until that ladder is spent, at which point it surfaces once through the ordinary stale reason carrying its spent-attempt count.
Rendered banner text only selects which stall it is; whether to act at all rests on the same semantic busy-state verdict used above plus a provably empty composer, so the exemption cannot fire on a working pane, one holding a question or permission dialog, or one with text already typed.
The banner must also sit immediately above the composer, and a due attempt against a pane byte-identical to the previous delivery is refused, so a recovered worker whose transcript still shows an old banner is never re-nudged.
[`configuration.md`](configuration.md#claude-stall-auto-resume-configauto-resume) owns the switch and tunables, and [`verification/supervision.md`](verification/supervision.md) records how far each of its three inputs is verified.
...
On a fork it also fetches the community `upstream` remote and reports how far the primary trails it, whether the merge would be clean, and an `upstream-merge:` verdict, read-only: a real catch-up is a merge task delivered through a branch and PR, never done inside the live primary checkout.

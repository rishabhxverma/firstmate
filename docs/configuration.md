...
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, and the primary turn-end guard treat a watcher beacon as stale; the turn-end guard's away-mode watcher-beat window is single-sourced from it
...
FM_PROJECT_ENV_SCAN_TIMEOUT=10   # seconds allowed per project for bootstrap's read-only PROJECT_ENV pool scan; non-numeric or 0 falls back to 10, and a timed-out project is skipped silently
...
FM_REFLECT_CMD=bin/fm-reflect.sh   # reflection capture run by fm-teardown.sh before cleanup; set empty to opt out of the harvest entirely
FM_REFLECT_TIMEOUT_SECS=20   # seconds allowed for that capture inside fm-teardown.sh
FM_REFLECT_EVENT_LINES=200   # last event-log lines bin/fm-reflect.sh captures
FM_REFLECT_COMMITS=100       # last branch commits bin/fm-reflect.sh captures

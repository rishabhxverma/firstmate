...
| `fm-update.sh`           | Guarded self-update of firstmate and local or remote secondmate homes, reconciling redundant divergence, classifying every live mate left on the target commit for restart or fallback nudge, and reporting read-only how far a fork trails its community `upstream` remote |
...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk and holder liveness) for every session-lock consumer, including fm-lock.sh, the Claude Stop auto-arm, and the tmux backend adapter |
...
| `fm-reflect.sh`          | Harvest one task's reflection material to `data/<id>/reflection.md` and queue its fold-in wake, read-only, before teardown destroys the records |

...
A guard that cries wolf every turn trains the operator to ignore the one time it is right, so the guard replaces that question while `state/.afk` exists, in either away or quiet mode, with a three-part liveness test, in both the default and `--claude` modes and before either block path.
...
3. `state/.last-watcher-beat` is fresher than the beat window, `FM_GUARD_GRACE` when set and otherwise the poll-derived grace described below, because the daemon starts a fresh one-shot watcher only after it finishes handling the previous wake and that handling can legitimately outrun a flat window; this proves the daemon's one-shot watcher child is still polling.
...
This stricter test is what the guard applies in place of a bare daemon-lock identity match: a live daemon that never reached its first housekeeping tick, or one whose watcher child never polls, still blocks.
With `state/.afk` absent the away test proves nothing and the strict watcher predicate is unchanged.
